...
const Sidebar = () => {
  ...
  //if inner width is greater than 768px it will be shown
    useEffect(() => {
    const handleResize = () => {
...
//if inner width is greater than 768px it will be hidden
  useEffect(() => {
  ...
  //it will handle which tech is selected
...
//calculate score
 const calculateScore = () => {
  ...
  const toggleSidebar = () => setIsSidebarOpen((prev) => !prev); //toggle sidebar
  ...
  if (!selectedTech || !selectedLevel) {
    console.log("Technology or level missing");
    return;
  }
  ...
  try {
    ...
    toast.error(
      err.response?.data?.message || "Result could not be saved"
    );
  }
};
          ...
          <div className={sidebarStyles.headerContent}>
            <div className={sidebarStyles.logoContainer}>
              ...
              <div>
                ...
                <p className={sidebarStyles.logoSubtitle}>Test your knowledge skills</p>
              </div>
            </div>
            ...
            <div key={tech.id} className={sidebarStyles.techItem} data-tech={tech.id}>
              <button onClick={()=>handleTechSelect(tech.id)}
               className={`${sidebarStyles.techButton} ${selectedTech=== tech.id 
                ? `${tech.color} ${sidebarStyles.techButtonSelected}` : `${sidebarStyles.techButtonNormal}`}`}>
                  ...
                  {selectedTech=== tech.id ? (<ChevronDown size={18} className="text-current"/>
                  ):(
                  ...
                  <div className={sidebarStyles.levelsContainer}>
                    ...
                    {levels.map((level)=>(
                      <button key={level.id} onClick={()=> handleLevelSelect(level.id)} className={`${sidebarStyles.levelButton} ${
                        ...
                        ? `${level.color} ${sidebarStyles.levelButtonSelected}` : sidebarStyles.levelButtonNormal
                      }`}>
